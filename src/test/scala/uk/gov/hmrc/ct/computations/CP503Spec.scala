...
class CP503Spec extends WordSpec with MockitoSugar with Matchers {
  ...
  class TestFixture {
    val mockBoxRetriever = mock[ComputationsBoxRetriever]
  } 
  
  "validateNotExceedingCP501" should {
    "return validation error if value is more that CP501" in new TestFixture {
    ...
    "return validation error if value is set but CP501 is not set" in new TestFixture {
      when(mockBoxRetriever.cp501()).thenReturn(CP501(None))
      CP503(Some(2)).validate(mockBoxRetriever) shouldBe Set(CtValidation(Some("CP503"), "error.CP503.propertyExpensesExceedsIncome", None))
    }


    "return no validation error if value is less that CP501" in new TestFixture {
      when(mockBoxRetriever.cp501()).thenReturn(CP501(Some(3)))
      CP503(Some(2)).validate(mockBoxRetriever) shouldBe Set()
    }
    "return no validation error if value is equal CP501" in new TestFixture {
      when(mockBoxRetriever.cp501()).thenReturn(CP501(Some(1)))
      CP503(Some(1)).validate(mockBoxRetriever) shouldBe Set()
    }
    "return no validation error if value is not set but CP501 is" in new TestFixture {
      when(mockBoxRetriever.cp501()).thenReturn(CP501(Some(1)))
      CP503(None).validate(mockBoxRetriever) shouldBe Set()
    }
    "return no validation error if value is not set and CP501 is also not set" in new TestFixture {
      when(mockBoxRetriever.cp501()).thenReturn(CP501(None))
      CP503(None).validate(mockBoxRetriever) shouldBe Set()
    }
    "return no validation error if value is 0 and CP501 is not set" in new TestFixture {
      when(mockBoxRetriever.cp501()).thenReturn(CP501(None))
