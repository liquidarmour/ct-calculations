  ...
  "Directors" should {
    ...
    "validate CoHo Only on AC8021 and AC8023" when {
      "cannot exist for statutory accounts answered no to AC8021" in {
        ...
        populatedDirectors.validate(boxRetriever) shouldBe Set(CtValidation(None, "error.Directors.AC8021.cannot.exist"))
      }
      ...
      "validate successfully for statutory accounts answered yes to AC8021" in {
        ...
        when(filingAttributesBoxValueRetriever.statutoryAccountsFiling()).thenReturn(StatutoryAccountsFiling(true))
        when(filingAttributesBoxValueRetriever.microEntityFiling()).thenReturn(MicroEntityFiling(false))
        populatedDirectors.validate(boxRetriever) shouldBe Set()
      }
    ...
    "validate Joint on AC8021 and AC8023" when {
      ...
      "cannot exist for micro entity accounts answered no to AC8021 amd AC8023" in {
        ...
        when(filingAttributesBoxValueRetriever.statutoryAccountsFiling()).thenReturn(StatutoryAccountsFiling(false))
        when(filingAttributesBoxValueRetriever.microEntityFiling()).thenReturn(MicroEntityFiling(true))
        populatedDirectors.validate(boxRetriever) shouldBe Set(CtValidation(None, "error.Directors.AC8023.cannot.exist"))
      }
    ...
    "validate successfully when no validation errors are present" in {
      ...
      directors.validate(boxRetriever) shouldBe empty
    }
    ...
    "validate director name length" in {
      ...
      directors.validate(boxRetriever) shouldBe expectedError
    }
    ...
    "validate under the correct conditions only" in {
      ...
      forAll(testTable) { (companiesHouseFiling: Boolean, hmrcFiling: Boolean, microEntityFiling: Boolean, ac8021: Option[Boolean], ac8023: Option[Boolean], expectedResult: Boolean) =>
        when(filingAttributesBoxValueRetriever.companiesHouseFiling()).thenReturn(CompaniesHouseFiling(companiesHouseFiling))
        when(filingAttributesBoxValueRetriever.hmrcFiling()).thenReturn(HMRCFiling(hmrcFiling))
        when(filingAttributesBoxValueRetriever.microEntityFiling()).thenReturn(MicroEntityFiling(microEntityFiling))
        when(boxRetriever.ac8021()).thenReturn(AC8021(ac8021))
        when(boxRetriever.ac8023()).thenReturn(AC8023(ac8023))

        directors.validate(boxRetriever).size > 0 shouldBe expectedResult
        directors.validate(boxRetriever).exists(_.errorMessageKey.contains("atLeast1")) shouldBe expectedResult
      }
    }
    ...
    "validate resignation date" in {
      ...
      directors.validate(boxRetriever) shouldBe Set(CtValidation(Some("ac8013.444"), "error.ac8013.required"))
    }
    ...
    "validate AC8005 and AC8007 cannot exist if ACQ8003" in {
      ...
      directors.validate(boxRetriever) shouldBe Set(
