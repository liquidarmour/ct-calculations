...
trait DirectorsReportEnabledCalculator {
  def calculateDirectorsReportEnabled(boxRetriever: Frs10xAccountsBoxRetriever): Boolean = {
    val isCoHoFiling = boxRetriever.filingAttributesBoxValueRetriever.companiesHouseFiling().value
    val isHmrcFiling = boxRetriever.filingAttributesBoxValueRetriever.hmrcFiling().value
    val isMicroEntityFiling = boxRetriever.filingAttributesBoxValueRetriever.microEntityFiling().value
